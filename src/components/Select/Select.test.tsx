...
describe('Select', () => {
  ...
  it('does not expand on click when disabled', () => {
    ...
    userEvent.click(button);
    expect(container.innerHTML).toEqual(
      '<div id="Select--1" class="Select Select--disabled"><button class="Clickable Select__value" id="Select__value--1" disabled="" aria-haspopup="listbox" aria-expanded="false" aria-labelledby="Select__label--1 Select__value--1"><label id="Select__label--1">Sort by:</label> Foo<span class="Select__icon"></span></button></div>'
    );
  });
